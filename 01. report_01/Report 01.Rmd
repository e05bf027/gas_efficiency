...
- admitted to the Intensive Care Unit (ICU) of UHG during the period 20/03/2020 - 20/03/2022
...
\
This represents my approach to wrangling the data to create a time-series for the admission of each patient. I have now designed a filter for the Metavision Query Wizard output that captures essentially any data we have available, with the caveats mentioned above.
...
# The observed sample
We now have a dataframe of the entire population who are in the study (n = `r nrow(population_prone)`). Of these, the patient height could be located for `r sum(population_prone$height_recorded == 'T')`, and not recorded or locatable for `r sum(population_prone$height_recorded == 'F')`. There are a few ways this missing data can be handled.

- delete the observations it is part of
- replace NA with ICNARC values
- impute with MICE

```{r isolate_population_df}
population_prone <- prone_data %>% 
  select(
    patient_id,
    gender,
    age,
...
## Summary of sampled population

\

Errors in this table, return later

```{r gtsummary_table_1_creation, eval=FALSE, include=FALSE}
...
```{r table_1 display, eval=FALSE, include=FALSE}
...
In this exploration, we will explore both how hard the ventilator is working at different times, and indices of oxygenation at various parts of the proning process to try and ascertain the direct effects on the patient. 
...
$$ VR = \frac{\dot{V}_\text{Emeasured} * PaCO_2}{100 * \text{PBW} * 5} $$
...
In order to examine this we will create a new df. New variables will be added to this df. These will be:
...
In addition, we will calculate changes in the following parameters from just before to the end of the proning session:
...
# repeat for fio2 =====================================================
prone_fio2_response <- prone_data %>% 
  select(
    patient_id,
    proning_session,
    proning_sequence,
  ...
  mutate(
    fio2_change = pre_unprone - pre_prone
  )%>% 
  rename(
    'fio2_pre_prone' = 'pre_prone',
    'fio2_pre_unprone' = 'pre_unprone'
  )

prone_fio2_response$id_col <- 1:nrow(prone_fio2_response) # adds unique col for later joining

# repeat for oxygenation factor ====================================================
...
prone_oxy_factor_response$id_col <- 1:nrow(prone_oxy_factor_response) # adds unique col for later joining


# repeat for aa_gradient ===========================================================
prone_aa_grad_response <- prone_data %>% 
  select(
    patient_id,
    proning_session,
    proning_sequence,
...
prone_aa_grad_response$id_col <- 1:nrow(prone_aa_grad_response) # adds unique col for later joining

# repeat for pao2 ==============================================================
prone_pao2_response <- prone_data %>% 
  select(
    patient_id,
    proning_session,
    proning_sequence,
  ...
  mutate(
    pa_o2_change = pre_unprone - pre_prone
  )%>% 
  rename(
    'pa_o2_pre_prone' = 'pre_prone',
    'pa_o2_pre_unprone' = 'pre_unprone'
  )

prone_pao2_response$id_col <- 1:nrow(prone_pao2_response) # adds unique col for later joining


# repeat for mean airway pressure =================================================
prone_paw_response <- prone_data %>% 
  select(
    patient_id,
    proning_session,
    proning_sequence,
  ...
  pivot_wider(
    names_from = proning_sequence,
    values_from = mean_airway_pressure_pb
  ) %>% 
  mutate(
    paw_change = pre_unprone - pre_prone
  )%>% 
  rename(
    'paw_pre_prone' = 'pre_prone',
    'paw_pre_unprone' = 'pre_unprone'
  )

prone_paw_response$id_col <- 1:nrow(prone_paw_response) # adds unique col for later joining


# repeat for total Hb =============================================================
prone_hb_response <- prone_data %>% 
  select(
    patient_id,
    proning_session,
    proning_sequence,
  ...
  pivot_wider(
    names_from = proning_sequence,
    values_from = total_haemoglobin
  ) %>% 
  mutate(
    hb_change = pre_unprone - pre_prone
  )%>% 
  rename(
    'hb_pre_prone' = 'pre_prone',
    'hb_pre_unprone' = 'pre_unprone'
  )

prone_hb_response$id_col <- 1:nrow(prone_hb_response) # adds unique col for later joining


# repeat for sao2 =============================================================
prone_sao2_response <- prone_data %>% 
  select(
    patient_id,
    proning_session,
    proning_sequence,
  ...
  pivot_wider(
    names_from = proning_sequence,
    values_from = sa_o2_systemic
  ) %>% 
  mutate(
    sao2_change = pre_unprone - pre_prone
  )%>% 
  rename(
    'sao2_pre_prone' = 'pre_prone',
    'sao2_pre_unprone' = 'pre_unprone'
  )

prone_sao2_response$id_col <- 1:nrow(prone_sao2_response) # adds unique col for later joining


# repeat for paco2 =============================================================
prone_pa_co2_response <- prone_data %>% 
  select(
    patient_id,
    proning_session,
    proning_sequence,
  ...
  mutate(
    pa_co2_change = pre_unprone - pre_prone
  )%>% 
  rename(
    'pa_co2_pre_prone' = 'pre_prone',
    'pa_co2_pre_unprone' = 'pre_unprone'
  )

prone_pa_co2_response$id_col <- 1:nrow(prone_pa_co2_response) # adds unique col for later joining


# repeat for end tidal co2 =======================================================
prone_etco2_response <- prone_data %>% 
  select(
    patient_id,
    proning_session,
    proning_sequence,
  ...
  pivot_wider(
    names_from = proning_sequence,
    values_from = end_tidal_co2_marquette
  ) %>% 
  mutate(
    et_co2_change = pre_unprone - pre_prone
  )%>% 
  rename(
    'etco2_pre_prone' = 'pre_prone',
    'etco2_pre_unprone' = 'pre_unprone'
  )

prone_etco2_response$id_col <- 1:nrow(prone_etco2_response) # adds unique col for later joining
...
# 3 - prone_aa_grad_response
# 4 - prone_oxy_factor_response
# 5 - prone_pao2_response
# 6 - prone_fio2_response
# 7 - prone_paw_response
# 8 - prone_hb_response
# 9 - prone_sao2_response
# 10 - prone_pa_co2_response
# 11 - prone_etco2_response
...
indices_response <- right_join(
  x = indices_response,
  y = prone_oxy_factor_response[, 4:7],
  by = 'id_col'
  )
...
rm(
  prone_pfr_response,
  prone_vr_response,
  prone_aa_grad_response,
  prone_etco2_response,
  prone_fio2_response,
  prone_hb_response,
  prone_pa_co2_response,
  prone_pao2_response,
  prone_paw_response,
...
#### CO2 and O2 responders

First we will calculate how many of these patients are 'oxygen responders' and 'CO2 responder'.

```{r respose_prone_1, echo=FALSE}
...
indices_response %>% 
  ...
  summarise(
    sum_o2_responders = sum(.$o2_responder, na.rm = T),
    perc_o2_responders = sum_o2_responders / ,
    sum_co2_responders = sum(indices_response$co2_responder, na.rm = T),
    perc_co2_responders = 
      sum(indices_response$co2_responder, na.rm = T) / nrow(indices_response)
  ) 
...
indices_response %>% 
  ...
  summarise(
    sum_o2_responders = sum(.$o2_responder, na.rm = T),
    perc_o2_responders = (o2_responder = 'TRUE' / (o2_responder = 'TRUE' + o2_responder = 'FALSE')),
    sum_co2_responders = sum(indices_response$co2_responder, na.rm = T),
    perc_co2_responders = 
      sum(indices_response$co2_responder, na.rm = T) / nrow(indices_response)
  ) 
